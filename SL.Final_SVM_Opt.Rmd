...
```{r}
corrplot(cor(absentee),  tl.cex = .6)
```
...
for (r in 1:R) {
  ...
  error_matrix[r,1] <- mean(yhat_tree!=absentee.te[,16])
  error_matrix[r,2] <- mean(yhat_rf!=absentee.te[,16])
  error_matrix[r,3] <- mean(yhat_lda!=absentee.te[,16])
  error_matrix[r,4] <- mean(as.numeric(yhat_qda)!=as.numeric(absentee.te[,16]))
  error_matrix[r,5] <- mean(yhat_svmC!=absentee.te[,16])
  error_matrix[r,6] <- mean(yhat_svmNu!=absentee.te[,16])
...
for (r in 1:R) {
  ...
  # store the errors in the matrix
  error_matrix[r,1] <- mean(yhat_svmC!=absentee.te[,16])
  error_matrix[r,2] <- mean(yhat_svmNu!=absentee.te[,16])
...
```{r}
# Label the columns
colnames(error_matrix) <- c('SVM-Nu','SVM-C')
#melt the matrix
error_matrix_melt <- melt(as.data.frame(error_matrix))
#rename the columns
colnames(error_matrix_melt) <- c('Method','Accuracy')
...
```{r}
# Label the columns
colnames(sensitivity_matrix) <- c('SVM-Nu','SVM-C')
#melt the matrix
sensitivity_matrix_melt <- melt(as.data.frame(sensitivity_matrix))
#rename the columns
colnames(sensitivity_matrix_melt) <- c('Method','Sensitivity')
...
```{r}
# Label the columns
colnames(specificity_matrix) <- c('SVM-Nu','SVM-C')
#melt the matrix
specificity_matrix_melt <- melt(as.data.frame(specificity_matrix))
#rename the columns
colnames(specificity_matrix_melt) <- c('Method','Specificity')
